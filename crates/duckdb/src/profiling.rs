...
impl ProfilingInfo {
    /// # Safety
    /// `info` must be a valid (or NULL) pointer obtained from [`libhaybarn_sys::duckdb_get_profiling_info`].
    /// Metric key/value string conversions are treated as fallible. NULL varchar results are skipped.
    fn from_raw(info: libhaybarn_sys::duckdb_profiling_info) -> Option<Self> {
        if info.is_null() {
            return None;
        }

        // Extract metrics
        let mut map = unsafe { libhaybarn_sys::duckdb_profiling_info_get_metrics(info) };
        let map_size = unsafe { libhaybarn_sys::duckdb_get_map_size(map) };

        let mut metrics = HashMap::<String, String>::with_capacity(map_size as usize);

        for i in 0..map_size {
            let mut key = unsafe { libhaybarn_sys::duckdb_get_map_key(map, i) };
            let mut val = unsafe { libhaybarn_sys::duckdb_get_map_value(map, i) };

            if let (Some(key_str), Some(val_str)) = (Self::value_to_string(key), Self::value_to_string(val)) {
                metrics.insert(key_str, val_str);
            }

            unsafe { libhaybarn_sys::duckdb_destroy_value(&mut key) };
            unsafe { libhaybarn_sys::duckdb_destroy_value(&mut val) };
        }

        unsafe { libhaybarn_sys::duckdb_destroy_value(&mut map) };

        // Extract children
        let child_count = unsafe { libhaybarn_sys::duckdb_profiling_info_get_child_count(info) };
        let mut children = Vec::with_capacity(child_count as usize);
        for i in 0..child_count {
            let child_info = unsafe { Self::from_raw(libhaybarn_sys::duckdb_profiling_info_get_child(info, i)) };
    ...
    fn value_to_string(value: libhaybarn_sys::duckdb_value) -> Option<String> {
        let ptr = unsafe { libhaybarn_sys::duckdb_get_varchar(value) };
        unsafe { libhaybarn_sys::DuckDbString::from_nullable_ptr(ptr) }
...
impl InnerConnection {
    ...
    pub fn get_profiling_info(&self) -> Option<ProfilingInfo> {
        let info = unsafe { libhaybarn_sys::duckdb_get_profiling_info(self.con) };
